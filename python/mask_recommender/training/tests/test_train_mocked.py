...
class TestDataPreprocessingMocked:
    ...
    def test_scale_data_mocked(self):
        ...
        with patch('sklearn.preprocessing.StandardScaler') as mock_scaler_class:
            ...
            # Create mock return value with correct shape (3 rows, 6 scalable columns)
            scalable_cols = [k for k, v in self.features_to_type_mapping.items() if v['scalable']]
            mock_scaler.fit_transform.return_value = np.random.randn(3, len(scalable_cols))
            
            df_copy = self.test_df.copy()
            
            # Mock the scale_data function
