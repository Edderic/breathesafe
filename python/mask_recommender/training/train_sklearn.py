...
from python.mask_recommender.sklearn_pipeline import build_sklearn_pipeline
...
def build_mask_metadata(df: pd.DataFrame) -> dict:
    """Aggregate style (mode) and perimeter (mean) per mask_id for inference."""
    meta = {}
...
def train_and_save(df: pd.DataFrame) -> dict:
    ...
    # Save mask metadata for inference (style, perimeter)
