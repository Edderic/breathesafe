...
export LOCAL_RECOMMENDER_WARM_MODELS="${LOCAL_RECOMMENDER_WARM_MODELS:-custom_lr}"

echo "==> Using AWS_PROFILE=${AWS_PROFILE}"
echo "==> Using RAILS_ENV=${RAILS_ENV}"

if [[ "${NO_DOWNLOAD:-0}" != "1" ]]; then
  echo "==> Skipping legacy NN artifact download; local recommender uses custom_lr"
else
