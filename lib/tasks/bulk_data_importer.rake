...
namespace :bulk_data do
  ...
  task :import => :environment do
    ...
    env   = ENV['env']   || ENV['ENV']   || ENV['HEROKU_ENVIRONMENT'] || Rails.env
    ...
    if read.blank? || write.blank? || user <= 0
      abort 'Usage: rake bulk_data:import style=Crash2.5 read=./in.csv write=./out.csv env=staging user_id=123 mode=validate testing_mode=N95'
    end
