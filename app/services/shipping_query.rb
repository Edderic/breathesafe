...
class ShippingQuery
  ...
  def self.usps_csv(shipping_date: nil)
    rows = find_users_who_are_awaiting_shipping_label
    ...
    ::CSV.open('usps_labels_to_create.csv', 'wb') do |csv|
      ...
      rows.each do |row|
        csv << row_generator(row: row, shipping_date: shipping_date)
      end
    end
  end
  ...
  def self.row_generator(
    ...
    shipping_date = DateTime.now if shipping_date.nil?

    {
      ...
      'Certificate #' => '',
      'Invoice #' => ''
    }.each_with_object([]) do |(_key, value), accum|
      accum << value
    end
  end
