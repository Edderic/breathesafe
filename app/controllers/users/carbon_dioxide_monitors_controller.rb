  ...
  class CarbonDioxideMonitorsController < ApplicationController
    ...
    def create_or_update
      ...
      if !authorized?
        ...
        message = if updated
                    "Updated data for CO2 Monitor with serial #{co2_monitor.serial}"
                  else
                    "Failed to update CO2 Monitor with serial #{co2_monitor.serial}"
                  end
        status = 200
      else
        c = false
        ...
        message = if !c.nil? && !u.nil?
                    "Successfully created CO2 Monitor with serial #{c.serial}"
                  else
                    "Failed to create CO2 Monitor with serial #{co2_monitor_data['serial']}"
                  end
        status = 200
      end
