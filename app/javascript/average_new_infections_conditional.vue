<template>
  <DrillDownSection
    ...
    :value='roundOut(numSusceptibles * conditionalRisk, 1)'
    :text='roundOut(numSusceptibles * conditionalRisk, 1)'
    :colorScheme='averageInfectedPeopleInterpolationScheme'
  >
    <tr>
      <td colspan=2>
        <div class='explainer'>
          <span>If we assume that there is one infector and that everyone stays there for {{selectedHour}} hour(s), the
          number of people that would be infected, on average, is <span
          class='italic'>{{roundOut(numSusceptibles * conditionalRisk, 1)}}</span>:

          </span>
          <div class='people-icons people justify-content-center align-items-center'>
            <PersonIcon
              backgroundColor='red'
              :amount='roundOut(numInfectedConditional, 0)'
            />
            <PersonIcon
              backgroundColor='green'
              :amount='numSusceptibles - roundOut(numInfectedConditional, 0)'
            />
          </div>
          ...
          <vue-mathjax formula='$s \cdot P(t \mid e) $'></vue-mathjax>, where <vue-mathjax formula='s'></vue-mathjax> is the number of susceptibles, and <vue-mathjax formula='$P(t \mid e, I=1)$'></vue-mathjax> is the probability of transmission given the evidence (i.e. what we know), assuming there is one infector (<vue-mathjax formula='$I = 1$'></vue-mathjax>).
          </p>
          <p>
          Assuming there are <vue-mathjax :formula='`$${numSusceptibles}$`'></vue-mathjax> susceptibles, and that the probability of transmission given the evidence is <vue-mathjax :formula='`$1 \\text{ in } ${roundOut(1 / conditionalRisk, 1)}$`'></vue-mathjax>, then the average new infections is
  ...
  computed: {
    ...
    mainFormula() {
      return `
        ...
        \\begin{equation}
        \\begin{aligned}
        A &= s \\cdot P(t \\mid e, I=1) \\\\
        &= ${this.numSusceptibles} \\cdot \\frac{1}{${round(1/this.conditionalRisk, 1)}} \\\\
        &\\approx ${round(this.numSusceptibles * this.conditionalRisk, 1)}
        \\end{aligned}
        \\end{equation}
        $$
      `
